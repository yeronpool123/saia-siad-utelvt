...
async function main() {
  ...
  const adminUser = await prisma.usuario.upsert({
    where: { email: 'admin@utelvt.edu.ec' },
    update: { nombre: 'Yeron', apellido: 'Cuero', passwordHash: await hashPassword('Admin123!'), activo: true, rol: Rol.SUPER_ADMIN, departamento: 'Tecnología de la Información' },
    create: { email: 'admin@utelvt.edu.ec', passwordHash: await hashPassword('Admin123!'), nombre: 'Yeron', apellido: 'Cuero', rol: Rol.SUPER_ADMIN, activo: true, departamento: 'Tecnología de la Información' },
  });
  const soporteUser = await prisma.usuario.upsert({
    where: { email: 'soporte@utelvt.edu.ec' },
    update: { nombre: 'Héctor', apellido: 'Sacón', passwordHash: await hashPassword('Admin123!'), activo: true, rol: Rol.SOPORTE_TI, departamento: 'Tecnología de la Información' },
    create: { email: 'soporte@utelvt.edu.ec', passwordHash: await hashPassword('Admin123!'), nombre: 'Héctor', apellido: 'Sacón', rol: Rol.SOPORTE_TI, activo: true, departamento: 'Tecnología de la Información' },
  });
  const soporte02User = await prisma.usuario.upsert({
    where: { email: 'soporte02@utelvt.edu.ec' },
    update: { nombre: 'Luis', apellido: 'Montaño', passwordHash: await hashPassword('soporte@admin123'), activo: true, rol: Rol.SOPORTE_TI, departamento: 'Tecnología de la Información' },
    create: { email: 'soporte02@utelvt.edu.ec', passwordHash: await hashPassword('soporte@admin123'), nombre: 'Luis', apellido: 'Montaño', rol: Rol.SOPORTE_TI, activo: true, departamento: 'Tecnología de la Información' },
  });
  const docenteUser = await prisma.usuario.upsert({ where: { email: 'docente@utelvt.edu.ec' }, update: { passwordHash: await hashPassword('Docente123!'), activo: true }, create: { email: 'docente@utelvt.edu.ec', passwordHash: await hashPassword('Docente123!'), nombre: 'María', apellido: 'García', rol: Rol.DOCENTE, activo: true, departamento: 'Facultad de Ingeniería' } });
  const estudianteUser = await prisma.usuario.upsert({ where: { email: 'estudiante@utelvt.edu.ec' }, update: { passwordHash: await hashPassword('Estudiante123!'), activo: true }, create: { email: 'estudiante@utelvt.edu.ec', passwordHash: await hashPassword('Estudiante123!'), nombre: 'Juan', apellido: 'Pérez', rol: Rol.ESTUDIANTE, activo: true, departamento: 'Ingeniería en Sistemas' } });
  console.log('5 usuarios sincronizados (3 ingenieros administradores)');
  ...
  const t1 = await prisma.ticket.upsert({
    where: { numero: 'TKT-2026-0001' },
    update: {},
    create: { numero: 'TKT-2026-0001', titulo: 'No puedo acceder al SIAD', descripcion: 'Desde ayer intento ingresar al sistema SIAD y me muestra un error de credenciales.', tipo: TipoSoporte.RESETEO_SIAD, prioridad: Prioridad.ALTA, estado: EstadoTicket.EN_PROCESO, userId: estudianteUser.id, asignadoAId: soporteUser.id, categoriaId: categorias[0].id, respuestaEn: new Date(Date.now() - 2 * 60 * 60 * 1000) }
  });
  const t2 = await prisma.ticket.upsert({
    where: { numero: 'TKT-2026-0002' },
    update: {},
    create: { numero: 'TKT-2026-0002', titulo: 'Solicitud de acceso a Microsoft Teams', descripcion: 'Necesito acceso a la plataforma Microsoft Teams para las clases virtuales.', tipo: TipoSoporte.ACCESO_PLATAFORMA, prioridad: Prioridad.MEDIA, estado: EstadoTicket.PENDIENTE, userId: docenteUser.id, categoriaId: categorias[2].id }
  });
  const t3 = await prisma.ticket.upsert({
    where: { numero: 'TKT-2026-0003' },
    update: {},
    create: { numero: 'TKT-2026-0003', titulo: 'La impresora del laboratorio no funciona', descripcion: 'La impresora HP LaserJet del laboratorio B-204 no imprime.', tipo: TipoSoporte.PROBLEMA_EQUIPO, prioridad: Prioridad.BAJA, estado: EstadoTicket.RESUELTO, userId: estudianteUser.id, asignadoAId: soporteUser.id, categoriaId: categorias[1].id, resolucion: 'Se reinstaló el driver de la impresora.', resueltoEn: new Date(Date.now() - 24 * 60 * 60 * 1000), cerradoEn: new Date() }
  });
  console.log('3 tickets sincronizados');

  console.log('');
  console.log('Seed completado exitosamente');
  console.log('Usuarios de prueba:');
  console.log('  Admin (Yeron Cuero):  admin@utelvt.edu.ec / Admin123!');
  console.log('  Soporte (Hector Sacon): soporte@utelvt.edu.ec / Admin123!');
  console.log('  Soporte 2 (Luis Montano): soporte02@utelvt.edu.ec / soporte@admin123');
